...
export async function PATCH(
  ...
  if (codigo_interno !== undefined) {
    ...
    const { data: dupList } = await supabaseCheck
      ...
      .neq("id", id)
      .limit(1);
    if (dupList && dupList.length > 0) return NextResponse.json({ error: "Ya existe un transportista con ese código interno" }, { status: 400 });
  ...
  if (dni !== undefined) {
    ...
    const { data: dupDniList } = await supabaseCheck
      ...
      .neq("id", id)
      .limit(1);
    if (dupDniList && dupDniList.length > 0) return NextResponse.json({ error: "Ya existe un transportista con ese DNI" }, { status: 400 });
  ...
  const { error } = await supabase.from("transportistas").update(updates).eq("id", id);
