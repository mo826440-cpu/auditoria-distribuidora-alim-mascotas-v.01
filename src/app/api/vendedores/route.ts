...
export async function POST(request: Request) {
  ...
  if (dupCodigoList && dupCodigoList.length > 0) {
    return NextResponse.json({ error: "Ya existe un vendedor con ese código interno" }, { status: 400 });
  }
  ...
  if (dupDniList && dupDniList.length > 0) {
    return NextResponse.json({ error: "Ya existe un vendedor con ese DNI" }, { status: 400 });
  }
