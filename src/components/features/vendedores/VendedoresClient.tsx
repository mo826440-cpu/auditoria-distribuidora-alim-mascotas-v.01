...
export function VendedoresClient({
  ...
  const formContent = (
    <>
      ...
      <div>
        <label className="block text-sm font-medium text-slate-300 mb-1">Nombre Vendedor *</label>
        <input
          type="text"
          value={formNombre}
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="rounded border-slate-300 text-primary-600 focus:ring-primary-500"
        />
        <label htmlFor="activo-vendedor" className="text-sm font-medium text-slate-300">
          Estado Activo
        </label>
      </div>
    </>
              ...
              {vendedores.length === 0 ? (
                ...
                vendedores.map((v) => (
                  <tr key={v.id} className="border-b border-slate-700 hover:bg-slate-700/50">
                    <td className="py-3 px-4 font-medium text-slate-200">{v.codigo_interno ?? "—"}</td>
                    <td className="py-3 px-4 text-slate-300">{v.nombre ?? "—"}</td>
                    <td className="py-3 px-4 text-slate-300">{v.contacto ?? "—"}</td>
                    ...
                    <td className="py-3 px-4">
                      <span
                        className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${
                          v.activo ? "bg-green-900/50 text-green-300" : "bg-slate-700 text-slate-400"
                        }`}
                      >
                        {v.activo ? "Activo" : "Inactivo"}
                      </span>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-slate-850 rounded-xl p-6 w-full max-w-md shadow-xl border border-slate-700">
            <h2 className="text-lg font-semibold text-slate-200">¿Eliminar vendedor?</h2>
